...
import dumpROMAttributes from './dump-rom-attributes'
...
export default function gameListEngine (
  ...
  map(gamesTargets, (e) => {
    // immediately bounce out if the rom is bios
    if (e._isbios === 'yes') {
      return
    }
    ...
    try { // generic try/catch to prevent errors from crashing the app
      ...
      // the display key is missing in some data, so we handle that at assignment
      let rotate = e.display && e.display._rotate ? e.display._rotate : '0'
      if (!e.display) {
        e.display = {}
      }
      if (!e.display._rotate) {
        e.display._rotate = '0'
      }
      ...
      console.log(`Error processing ROM "${e._name}": ${error.message}`)
      // dump attributes of offending rom
      console.log(dumpROMAttributes(e))
